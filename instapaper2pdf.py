...
# -----------------------------------------------------------------------------
def generate_qrcode(article_id, article_url):
    ...
    return qrcode_datablock

if __name__ == '__main__':

    ## Configuration
    articles_to_fetch = int(sys.argv[1]) if len(sys.argv) > 1 else 25
    ...
    with requests.session() as s:
        ...
        for l in article_links[0:articles_to_fetch]:
            ...
            if len(metadata) == 0:
                ...
                qrcode_datablock = generate_qrcode(article_id, article_url)
    ...
    logger.info(f'Converting to PDF with Playwright')
    with sync_playwright() as p:
        browser = p.chromium.launch()
        page = browser.new_page()
        page.set_content(final_html, wait_until='networkidle')
        page.pdf(
            path=f'output/{output_file_name}.pdf',
            format='A4',
